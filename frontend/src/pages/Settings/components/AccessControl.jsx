...
const AccessControl = () => {
  ...
  const permissionsGroups = [
    ...
    },
    {
      ...
      permissions: [
        { name: 'Upload Trackers', description: 'Bulk data upload and tracking systems', tags: ['UPLOAD'] },
        { name: 'Budget Upload', description: 'Financial forecasting and budget management', tags: ['UPLOAD'] },
  ...
  const handleTogglePermission = (moduleName, subPermId = null) => {
    ...
    if (subPermId) {
      // Toggle a specific sub-permission (e.g. Employee Master:ADD)
      ...
      if (isEnabled) {
        ...
        updatedPermissions = currentPermissions.filter(p => p !== moduleName && !p.startsWith(`${moduleName}:`));
      } else {
        ...
        const subPerms = moduleObj?.subPermissions?.map(sp => `${moduleName}:${sp.id}`) || [];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
      {/* Notification Banner */}
      {notification.show && (
        <div className={`fixed bottom-8 right-8 px-6 py-4 rounded-2xl shadow-2xl z-[200] flex items-center gap-3 animate-in fade-in slide-in-from-right-8 duration-300 border ${
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4 pr-8">
                       <div className={`p-3 rounded-xl shadow-sm border ${
                          selectedRole?.id === role.id ? 'bg-indigo-50 border-indigo-100 text-indigo-600' : 'bg-slate-100 border-slate-200 text-slate-400'
                       }`}>
              ...
              <div className="p-8 space-y-6">
                 ...
                 <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      className="w-full min-h-[120px] p-6 rounded-2xl border border-slate-100 bg-slate-50 focus:bg-white focus:border-indigo-600 outline-none transition-all font-medium text-slate-700 resize-none"
                    />
                 </div>
              </div>
